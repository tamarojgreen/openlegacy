				...
				try {
	    			...
	    			if (ref !=null) {
	    				...
	    				if (view != null) {
	    					...
	    					PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().showView(MAP_VIEW_ID, project.getName(), IWorkbenchPage.VIEW_ACTIVATE );
	    				}	
	    			}
	    			
	    			if (mapView == null) {
	    				mapView = (EntityMapView)PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().showView(MAP_VIEW_ID, project.getName(), IWorkbenchPage.VIEW_VISIBLE
	    						);
	    				mapView.setProject(project);
	    			}
