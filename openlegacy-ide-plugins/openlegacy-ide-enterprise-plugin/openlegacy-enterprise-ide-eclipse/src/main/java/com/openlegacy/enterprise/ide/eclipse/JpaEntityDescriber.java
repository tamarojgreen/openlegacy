 ...
 * 
 */
public class JpaEntityDescriber implements ITextContentDescriber {
	...
	private static String doDescribe(Reader contents) {
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (!commentStarted && commentEnded && line.contains(MessageFormat.format("@{0}", Entity.class.getSimpleName()))) {
					if (line.matches("\\s*@" + Entity.class.getSimpleName() + ".*")) {
						return line;
					}
				}
