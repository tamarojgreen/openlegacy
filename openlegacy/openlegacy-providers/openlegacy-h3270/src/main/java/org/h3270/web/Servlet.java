...
import org.h3270.render.Renderer;
 ...
 * @version $Id: Servlet.java,v 1.36 2008/11/21 14:50:33 spiegel Exp $
 */
public class Servlet extends AbstractServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (state.getTerminal(request) != null) {
			...
			Renderer r = (state.useRenderers() && engine.canRender(s)) ? (Renderer)engine : (Renderer)basicRenderer;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getParameter("connect") != null) {
			String hostname = (targetHost == null) ? request.getParameter("hostname") : targetHost;
		...
		} else if (request.getParameter("dumpfile") != null && !request.getParameter("dumpfile").equals("")) {
			String filename = new File(getRealPath("/WEB-INF/dump"), request.getParameter("dumpfile")).toString();
	...
	private void connect(HttpServletRequest request, SessionState state, String hostname) throws IOException,
		...
		try {
			...
			if (hostname.startsWith("file:")) {
				String filename = new File(getRealPath("/WEB-INF/dump"), hostname.substring(5)).toString();
	...
	/**
	 * If any preferences have been changed by the user in this request, take the appropriate action.
	 */
	private void handlePreferences(SessionState state, HttpServletRequest request, HttpServletResponse response)
			throws IOException {
		...
		if (modified) {
			Cookie cookie = new Cookie(SessionState.COOKIE_NAME, state.getSavedState());
	...
	/**
	 * Gets the input field parameters from the request and fills them into the Fields of the Screen, then submits them back to
	 * s3270.
	 */
		...
		if (s.isFormatted()) {
			for (Iterator i = s.getFields().iterator(); i.hasNext();) {
				...
				if (f instanceof InputField) {
					if (!f.isMultiline()) {
						String value = request.getParameter("field_" + f.getStartX() + "_" + f.getStartY());
						if (value != null) {
							((InputField)f).setValue(value);
						}
					} else { // multi-line field
						for (int j = 0; j < f.getHeight(); j++) {
							String value = request.getParameter("field_" + f.getStartX() + "_" + f.getStartY() + "_" + j);
							if (value != null) {
			...
			state.getTerminal(request).submitUnformatted(request.getParameter("field"));
		}
	}

	/**
	 * Returns the SessionState object that is associated with the given request, creates the object if it doesn't already exist.
	 */
	private SessionState getSessionState(HttpServletRequest request) throws IOException {
