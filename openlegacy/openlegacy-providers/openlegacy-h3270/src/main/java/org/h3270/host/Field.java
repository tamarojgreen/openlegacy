 ...
 * @version $Id: Field.java,v 1.22 2008/11/21 14:47:22 spiegel Exp $
 */
public class Field {
	...
	public Field(Screen screen, byte fieldCode, int startx, int starty, int endx, int endy, int extendedColor,
			int extendedHighlight) {
		this.screen = screen;
		this.startx = startx;
		this.starty = starty;
		this.endx = endx;
	...
	public Field(Screen screen, byte fieldCode, int startx, int starty, int endx, int endy) {
		this(screen, fieldCode, startx, starty, endx, endy, ATTR_COL_DEFAULT, ATTR_EH_DEFAULT);
	}

	/**
	 * Returns the x coordinate (column) at which this Field begins. Column numbers start at zero. The number returned is the
	 * position of the Field's first character, not of the control character that opens the Field.
	 */
	...
	/**
	 * Returns the y coordinate (row) in which this Field begins. Row numbers start at zero, increasing downward from the top.
	 */
	...
	/**
	 * Returns the x coordinate (column) at which this Field ends. Column numbers start at zero. The number returned is the
	 * position of the Field's last character, not of the control character that terminates the Field.
	 */
	public int getEndX() {
		return endx;
	}
	...
	/**
	 * Returns the y coordinate (row) in which this Field ends. Row numbers start at zero, increasing downward from the top.
	 */
	public int getEndY() {
		return endy;
	}
	...
	/**
	 * Returns the width (number of characters) of this Field. This does not include the control characters that delimit the
	 * Field.
	 * 
	...
	/**
	 * Returns the height of this Field, in lines. If the control characters at the start or the end of this Field are at the end
	 * or the start of the preceding or next line, respectively, then those lines are not incluced in the height count -- only the
	 * actual text of the Field counts.
	 */
	...
	/**
	 * Returns the current value of this Field. This does not include the control character that starts the field, and it does not
	 * include leading or trailing newlines. If this Field is a multi-line field, the individual lines are separated by newlines.
	 */
	...
	/**
	 * Returns the textual representation of this Field. Unlike getValue(), this methods prepends a blank for the opening field
	 * attribute, and adds newline characters at the beginning and the end as needed. Calling this method for each Field of a
	 * Screen, and concatenating the results, creates a full printable, textual representation of the screen.
	 */
	...
	/**
	 * If this Field has an extended color, returns the index of that color (0xf1 through 0xf7). If this Field does not have an
	 * extended color assigned, returns zero.
	 */
	...
	/**
	 * If this field has extended highlighting, returns the index of that highlighting scheme. If this Field does have extended
	 * highlighting, zero is returned.
	 */
