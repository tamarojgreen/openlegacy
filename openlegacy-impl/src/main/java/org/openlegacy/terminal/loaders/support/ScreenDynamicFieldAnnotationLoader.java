...
public class ScreenDynamicFieldAnnotationLoader extends AbstractFieldAnnotationLoader {
	...
	protected SimpleDynamicFieldTypeDefinition fillTypeDefinition(Annotation annotation, FieldDefinition fieldDefinition,
			String fieldName) {
	...
	public void load(EntitiesRegistry entitiesRegistry, Field field, Annotation annotation, Class<?> containingClass,
		...
		if (screenEntityDefinition != null) {
			fieldDefinition = (SimpleScreenFieldDefinition)screenEntityDefinition.getFieldsDefinitions().get(fieldName);
			dynamicFieldDefinition = fillTypeDefinition(fieldAnnotation, fieldDefinition, fieldName);
		} else {
			...
			if (screenPart != null) {
				...
				fieldDefinition = (SimpleScreenFieldDefinition)screenPart.getFieldsDefinitions().get(fieldName);
				dynamicFieldDefinition = fillTypeDefinition(fieldAnnotation, fieldDefinition, fieldName);
			}
		}
